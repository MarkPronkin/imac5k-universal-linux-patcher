...
| iMac Retina 5K, 27-inch | 2017 | `iMac18,3` | ✅ **Verified** | ✅ **Verified** | ✅ **Verified** | ➖ N/A | ✅ **Verified** | ✅ **Verified: repeated s2idle sleeps** | ✅ **Verified** | ✅ **Verified: iGPU and brightness**; Limine only; untested with suspend |
...
| **Omarchy** | ✅ **Verified** | ✅ **Verified** | ✅ **Verified** | ✅ **Verified: Hyprland** | ✅ **Verified: repeated s2idle sleeps (iMac18,3)** | ✅ **Verified** | ✅ **Verified: iGPU and brightness** |
...
| 🔆 **Brightness** | The firmware hands Linux a backlight that accepts writes and dims nothing, because it only drives the panel for macOS. | ✅ Dims in macOS mode (verified on iMac18,3); the full range needs the rebuilt ACPI table |
...
- 🌗 **macOS mode has never been combined with working suspend** — the module itself is verified on the iMac18,3 (iGPU, Quick Sync, brightness), but it adds a second PCI device with runtime PM to the sleep path, leaves `i2c_i801` unbound and swaps in a recompiled ACPI table. Upstream masks sleep entirely and names `i915` among its suspend suspects, so nobody has run the two together. Re-test sleep after applying it, with [the recovery paths](docs/macos-mode.md#recovery) to hand.
