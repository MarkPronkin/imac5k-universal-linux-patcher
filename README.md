...
- 📺 **VP9 in Firefox and Chromium is still CPU-decoded** — with macOS mode the iGPU decodes VP9, but those two render on the compositor's GPU, the Radeon, which has no VP9 decoder, and neither can import frames decoded on the other chip: Firefox closed cross-GPU decode as WONTFIX, Chromium fails the import. Everything built on GStreamer *can*, because its `va` plugin is multi-device: a WebKit browser (Epiphany) with `gst-plugin-va` installed decodes VP9 on the iGPU and displays it through the Radeon, verified here. So does `mpv --hwdec=vaapi-copy --vaapi-device=/dev/dri/intel-render`. See [docs/macos-mode.md](docs/macos-mode.md#except-in-a-webkit-browser-where-it-works). AV1 is on neither chip; that one is a plain hardware limit.
