...
- 📺 **VP9 in a browser is still CPU-decoded** — with macOS mode the iGPU *does* decode VP9 (verified with ffmpeg and mpv), but Firefox and Chromium render on the compositor's GPU, the Radeon, which has no VP9 decoder, and neither can import frames decoded on the other chip: Firefox closed cross-GPU decode as WONTFIX, Chromium fails the import. Players that choose their own device do get it — `mpv --hwdec=vaapi-copy --vaapi-device=/dev/dri/intel-render`. See [docs/macos-mode.md](docs/macos-mode.md#why-vp9-is-still-software-decoded-in-the-browser). AV1 is on neither chip; that one is a plain hardware limit.
