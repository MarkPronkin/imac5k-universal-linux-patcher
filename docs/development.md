...
| `test_suspend.py`, `test_tb_sleep_hook.py`, `test_wifi_sleep_hook.py` | Both sleep modules on the Omarchy and Fedora backends: sleep target masks, the s2idle sleep drop-in and old boot-argument cleanup; `suspend` n/a off the iMac18,3, with an earlier release's leftovers reported partial and taken back out; `t2suspend`'s T2 rules (t2bce bound, no T2 unload hooks, only the Thunderbolt hook) against a fake PCI tree; the iMac18,3 USB controller fix (DKMS build, boot load, detection, removal, Secure Boot) against fake DKMS and sysfs; the Thunderbolt and Wi-Fi sleep hooks against a fake sysfs tree |
