...
| libva-utils (optional) | libva-utils | — |
...
no application can use it for video. `libva-utils` provides `vainfo`, which is
how `scripts/imac-igpu-check` reports what each chip can decode and encode.
